...
// MLP 가중치 및 바이어스
#include "MLP_32_16_95.43_10.h" // 가중치, 바이어스를 포함한 헤더 파일 이름
...
// 초기 화면 설정
void drawBox() {
  tft.drawRect(BOX_X, BOX_Y, BOX_SIZE, BOX_SIZE, TFT_BLACK);
}
...
void drawButtons() {
  ...
  // Reset 버튼
  tft.fillRect(BUTTON_RESET_X, BUTTON_RESET_Y, BUTTON_WIDTH, BUTTON_HEIGHT, TFT_RED);
  tft.setTextColor(TFT_WHITE);
  tft.drawCentreString("Reset", BUTTON_RESET_X + BUTTON_WIDTH / 2, BUTTON_RESET_Y + 2, FONT_SIZE);

  // Print 버튼
  tft.fillRect(BUTTON_PRINT_X, BUTTON_PRINT_Y, BUTTON_WIDTH, BUTTON_HEIGHT, TFT_GREEN);
  tft.setTextColor(TFT_WHITE);
  tft.drawCentreString("Print", BUTTON_PRINT_X + BUTTON_WIDTH / 2, BUTTON_PRINT_Y + 2, FONT_SIZE);
}
